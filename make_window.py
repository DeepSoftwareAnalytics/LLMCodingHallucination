...
class BaselineWindowMaker:
    ...
    def build_window(self):
        code_windows = []
        task_ids = {task['metadata']['task_id'].split('/')[0] for task in self.tasks}
        print(task_ids & {self.repo})
    ...
    def build_window(self, type='centered'):
        ...
        for prediction in self.predictions:
            ...
            for sample in [prediction['choices'][i]['text'] for i in range(len(prediction['choices']))]:
                ...
                # end_line_no = min(len(sample_lines), line_no + self.window_size - delta_size)
...
class MakeWindowWrapper:
    def __init__(self, benchmark, repos, window_sizes, slice_sizes):
        ...
        elif benchmark == CONSTANTS.codereval_benchmark:
            self.task_file_path = FilePathBuilder.codereval_benchmark
