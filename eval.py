...
def run_RG1_and_oracle_method(benchmark, repos, window_sizes, slice_sizes):
    ...
    # mode = CONSTANTS.gt
    # output_file_path = f"prompts/gt-one-gram-ws-20-ss-2.jsonl"
...
def run_RepoCoder_method(benchmark, repos, window_sizes, slice_sizes, prediction_path):
    ...
    tokenizer = CodexTokenizer
    output_file_path = f"prompts/{MODEL_NAME}-{'lsp' if CALL_LSP else 'nolsp'}/repocoder-one-gram-ws-20-ss-2.jsonl"
...
CoderEval_ROOT = "/home/wangchong/Workspace/Coder-LSP/CoderEval"
N = 1
...
def predict(tasks):
    ...
    if not CALL_LSP:
        ...
        for idx, task in enumerate(tasks, 1):
            _id = task["metadata"]["task_id"].split("/")[-1]
            _ids.append(_id)
            repo2insts[task["metadata"]["lsp-repo"]].append((idx, task))
        for repo, insts in tqdm(repo2insts.items(), total=len(repo2insts), ascii=True, desc="Evaluation"):
            ...
            for beg, end in batch_ranges:
                ...
                batch = [
                    {
                        "path": f"repos/{task['metadata']['lsp-path']}",
                        "context": task['metadata']['lsp-context'],
                        "line": task['metadata']['lsp-line'],
                        "column": task['metadata']['lsp-column'],
                        "docstr": clean_docstring(task['metadata']['docstring']),
                        "signature": task['metadata']['signature'],
                        "prefix": task['prompt'],
                    }
                    for _, task in insts[beg:end]
                ]
                # logging.info(f"====================================")
...
if __name__ == '__main__':
    ...
    with Path(f"{CHECKPOINT}/trainer_state.json").open("r") as f:
        ...
        CHECKPOINT = f"/home/wangchong/Workspace/Coder-LSP/{CHECKPOINT}"


    init_log(f"{DIR}/testing-CoderEval-RepoCoder/testing.log", logging.INFO)
    PRED_RESULT_FILE_R1 = f"{DIR}/testing-CoderEval-RepoCoder/predictions_r1.jsonl"
    PRED_RESULT_FILE_R2 = f"{DIR}/testing-CoderEval-RepoCoder/predictions_r2.jsonl"

    id2task = dict()
    with Path(f"datasets/CoderEval.jsonl").open("r") as f:
        for line in f:
    ...
    # ROUND 2
    run_RepoCoder_method(CONSTANTS.codereval_benchmark, repos, window_sizes, slice_sizes, PRED_RESULT_FILE_R1)
    tasks = []
    with Path(f"prompts/{MODEL_NAME}-{'lsp' if CALL_LSP else 'nolsp'}/repocoder-one-gram-ws-20-ss-2.jsonl").open("r") as f:
        for line in f:
            d = json.loads(line.strip())
            _id = d["metadata"]["task_id"].split("/")[-1]
            task = id2task[_id].copy()
    ...
    for _id, pred in predictions:
        ...
        task["generate_results"] = [pred]
        lines.append(json.dumps(task))
    with Path(PRED_RESULT_FILE_R2).open("w") as f:
        f.write("\n".join(lines))
    
    run_test_cases(PRED_RESULT_FILE_R2)
